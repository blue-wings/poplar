...
import com.bserver.appInterface.AbstractController;
...
public class AppClassScanner {
    ...
    public void scan() throws ClassNotFoundException, IllegalAccessException, InstantiationException {
        ...
        String appBasePath = url.getPath()+"/"+ "com/bserver/app";
        String[] extensions = {"class"};
        Collection<File> files = FileUtils.listFiles(new File(appBasePath), extensions, true);
         for(File file : files){
             String filePath = file.getAbsolutePath();
             String classpath = filePath.substring(filePath.indexOf("/com/bserver/app/") + 1, filePath.lastIndexOf("."));
